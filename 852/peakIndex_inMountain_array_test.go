...
func Test_peakIndexInMountainArray3(t *testing.T) {
	type args struct {
		A []int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := peakIndexInMountainArray3(tt.args.A); got != tt.want {
				t.Errorf("peakIndexInMountainArray3() = %v, want %v", got, tt.want)
			}
		})
	}
}