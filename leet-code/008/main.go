...
模板：func myAtoi(s string) int {}
*/

/**
1：定义变量start，p表示遍历str的游标，n=len(str), negative表示正负数
2：忽略空格后，p==n, 则返回0
3：判断+-，negative赋值
4：清除首位0
5：此时开始遍历数字，start=p。遍历数字后p=start，说明没有有效数字，返回0
6：如果p-start>10（2^31是10位数），说明超过极值，返回0
7：遍历字符串str，将其转为整型num
8：如果转换后超过极值，则返回极值
